# Project AEGIS-PHARMA — Governing Project Plan (V2)

> **Supersedes** `AEGIS_PROJECT_PLAN_FINAL.md` (v1.2) and the four prior drafts. FINAL retained for audit. **This V2 file is the schedule and delivery SoT from ratification.**  
> Validation basis: full package SoT parse (DoD, workshop, scope, eval, evidence standard, continuity CSV, Cursor assets) + AppFlowy docs (AGENTS Engineering Principles + Cursor AI Master Build Guide). Gap closures **G-24…G-38** in §24.
...
| Field | Entry |
|---|---|
| Title | Governing Project Plan V2 — dual-track capstone + optional production-ready RC |
...
| Principles addendum | Cursor eval §§11.8–11.9 · AppFlowy engineering principles · Cursor operating method §15.1 · Repo/AppFlowy gap closures §24 |
| Dual outcome | **(A)** Capstone defence-ready (G1–G8 / package DoD) · **(B)** Production-ready RC (requires G9) — **G9 is plan DECISION, not package completion SoT** |
| Related | `REPO_APPFLOWY_VALIDATION_V2.md`; `APPFLOWY_PRINCIPLES_VALIDATION.md`; `AEGIS_FDE_DAYWISE_TASK_DISTRIBUTION.md`; package `WORKSHOP_DEPLOYMENT_PLAN.md`, `DEFINITION_OF_DONE.md`, `PACKAGE_SCOPE_AND_ASSUMPTIONS.md`; `.cursor/rules/pharma-fde.mdc`; `.cursor/rules/engineering-principles.mdc` |
| Living location | `ProjectPlan/AEGIS_PROJECT_PLAN_V2.md` — mirror to `submission/artefacts/ProjectPlan/` at kickoff |
| Timeline PNG | `ProjectPlan/AEGIS_PROJECT_TIMELINE_V2.png` (legacy alias `AEGIS_PROJECT_TIMELINE_FINAL.png`; regenerate via `_render_timeline_gantt.py`) |
| Swimlanes | `ProjectPlan/AEGIS_PROJECT_SWIMLANE_V2.md` — P1–P5 seat lanes × P0–P9 / G1–G9 + Option B+ D1–D14 |
| Swimlane Excel | `ProjectPlan/AEGIS_PROJECT_SWIMLANE_TRACKER_V2.xlsx` (phases/days as rows; seats **FDE1–FDE5**; regenerate/validate via `_build_swimlane_tracker_v2.py`) |
| Day-wise FDE task matrix | `ProjectPlan/AEGIS_FDE_DAYWISE_TASK_DISTRIBUTION.md` + **`AEGIS_FDE_DAYWISE_TASK_TRACKER_V2.xlsx`** (v1 xlsx superseded) |
| Briefings (owed) | PPTX decks are **owed artefacts**, not yet present — create under `submission/artefacts/ProjectPlan/` before G8 |
...
5. **Offline / AI-disabled (FACT — workflow-specific):** deterministic offline path is Must. Continuity from `data/continuity_requirements.csv`: **batch & supply `max_ai_outage_days=14`**; **PV `max_ai_outage_hours=0`** (manual runbook still required; do not claim a 14-day PV AI-disabled window). Kill switch / safe degradation required on Track A and Track B.
...
8. **Dual-track honesty (DECISION):** package completion SoT = Track A (G1–G8 / 40h / DoD). “Production-ready” language requires Track B (P9 / G9, +16–24h). Never claim production-ready from Track A alone. G9 is optional for score.
9. **Agent freeze (DECISION):** agents / model-inference features OFF until **G4 PASS** and controls merged; deterministic core always on.
10. **Facilitator independence (FACT):** no reliance on hidden answer keys, later injects, private credentials, or undocumented oral instructions; assumptions logged same day (`WORKSHOP_DEPLOYMENT_PLAN.md`).
11. **Deliberate ambiguity (FACT):** do not “clean” conflicting IDs/times/authorities/states; preserve `referenced_missing` / `untrusted` / `draft` / `superseded` as challenge conditions (`PACKAGE_SCOPE_AND_ASSUMPTIONS.md`).
12. **Stack freedom (FACT):** KG / agent / vector DB / LLM are **not mandatory** unless justified; any stack must still ship locked deps (or stdlib), offline/mocked mode, export, and migration path.
13. **Regulatory framing (FACT):** every regulatory conclusion states jurisdiction, intended purpose, accountable role, system boundary, risk classification, and assumptions.
14. **Hard gates ≠ score (FACT):** hard gates from `SCORING_MODEL.md` / rubric apply **independently of the 180-point score**.
15. **Engineering principles (DECISION + AppFlowy):** always-apply `.cursor/rules/engineering-principles.mdc` alongside `pharma-fde.mdc`; do **not** duplicate full text as root `AGENTS.md`.
...
Prior gaps G-01…G-20 remain closed as in V1. New merge decisions: **G-21** role model = V2 seats (P4=GxP/ISO, P5=Sec/Eval); **G-22** gate SoT = G1–G9 with CP* mapping; **G-23** three budget labels (40h / realistic buffer / +P9). Historical merge notes were never checked in as `MERGE_AUDIT.md`; V2 validation record is `REPO_APPFLOWY_VALIDATION_V2.md` (§24 G-24…G-38).
...
| No offline deterministic path, or continuity windows violate `continuity_requirements.csv` (batch/supply 14d; PV 0h) | **PAUSE** | INJ-082 |
...
| `runbooks/` | 4 | **Under `submission/runbooks/`:** SETUP, OPERATIONS, INCIDENT, AI_DISABLED (owed). Package-root `runbooks/PARTICIPANT_RUNBOOK.md` + `REPO_EXECUTION.md` are challenge guides — not substitutes | P5+P1 | P5–P7 |
| `scripts/` | 5 name-match Must | setup, run, test, evaluate, reset (**plus evidence-export** as DoD/REPO Must — 6th capability; may be `export.*` or documented export mode on `evaluate`/`run`) | P3 | P1 skeleton → P7 |
| `app/` | 1 | Demonstrator (safe DOM — no `innerHTML`) | P3 | P5 |

`submission_manifest.csv` columns exactly: `path,owner,version,status,sha256`.

### 7.3 Machine-readable evidence schemas (FACT: `SUBMISSION_EVIDENCE_STANDARD.md`)

**`test_results.json`** — each record includes at minimum: `suite`, `test_id`, `requirement_ids` / `control_ids`, `result`, `timestamp`, `runtime_mode`, `evidence_path`.

**`evaluation_results.json`** — each record includes at minimum: `dataset` / `cohort`, `grader`, `threshold`, `observed`, `gate_result`, `evidence_path`.

**Runbooks** (SETUP / OPERATIONS / INCIDENT / AI_DISABLED): each documented command states prerequisites, inputs, expected outputs, failure handling, and reset/rollback.

**Secrets ban (FACT):** no credentials, live personal data, proprietary source records, or undeclared external runtime dependencies in the submission.
...
**P9 / G9 validation checklist (AEGIS-adapted from AppFlowy production-readiness review — not Gantt product):** offline operation · setup/run/test/evaluate/reset/export · contract + prohibited suites · eval gates · no secrets · no fabricated completion claims · accessibility smoke · performance/budget soak within declared caps · dependency license/vuln note · hard gates on RC · residual risks documented.
...
**PNG chart (SoT visual):** `ProjectPlan/AEGIS_PROJECT_TIMELINE_V2.png` — dual-track Gantt (Track A 40h + Track B P9 mid-20h of 16–24h range) with gates G1–G9, plus Option B+ D1–D14 panel. Regenerate via `python -B ProjectPlan/_render_timeline_gantt.py`. Mirror under `submission/artefacts/ProjectPlan/` when that tree exists.

**Seat swimlanes:** `AEGIS_PROJECT_SWIMLANE_V2.md` — P1–P5 / V·D·A·Q·S lanes across P0–P9, gate RACI, workflow RACI, and Option B+ D1–D14.

Calendar dates are a scale device (1 day ≈ 1 elapsed hour from arbitrary epoch). Read hour labels, not axis dates.

```mermaid
gantt
    title AEGIS-PHARMA V2 — Track A 40h + optional P9
...
**Eval deliverables Must (FACT: `EVALUATION_PLAN.md`):** golden + edge + adversarial + failure-recovery sets; deterministic graders where possible; **calibrated human rubric**; **LLM-judge controls if an LLM judge is used**; **regression history**; evidence that failed gates block release.
...
**CSV schema (`submission/evidence/finops_delivery_log.csv`):** `date,stage_or_task,owner,chat_sessions,files_attached,agent_iterations,terminal_commands,files_modified,failed_attempts,full_suite_runs,outcome,notes`.

**Formula:** usage metrics + efficiency signals + outcome tracking → better FinOps decisions (feeds artefact 23 and defence element 9). Avoided inference and offline deterministic path remain first-class cost controls. Do **not** optimize cost by weakening validation (AppFlowy FinOps discipline).
...
**If time short:** protect hard-gate artefacts **06,09,12,13–18,22,24,25,27** plus **RUB-08 code quality** (`src`/`tests`/`app`) and **RUB-13 eval**. Cut depth on artefact **08** (KG decision), 19,20,23,26,28,29 before cutting workflows. **Never confuse “cut depth on 08” with cutting RUB-08 engineering.**
...
| Asset | Use at |
|---|---|
| `.cursor/rules/pharma-fde.mdc` | Always-on GxP / hard-gate guardrails |
| `.cursor/rules/engineering-principles.mdc` | Always-on general engineering principles (AppFlowy Cursor-tailored; do not also duplicate as root `AGENTS.md`) |
...
**Cursor-oriented engineering evidence is Must** (or recorded equivalent if not using Cursor) — cite rules/skills/commands in ADR 11; save agent review outputs under `submission/evidence/` (`ARTEFACT_EXPECTATIONS.md`). Not a MoSCoW “Could”.

### 15.1 Cursor operating method (AppFlowy Master Build Guide §21 — AEGIS-adapted)

1. Keep `engineering-principles.mdc` + `pharma-fde.mdc` as always-on roots.
2. **Plan Mode first** for multi-file / architectural / gate-affecting work; approve before Agent implement.
3. Do **not** ask Cursor to deliver the entire capstone or an entire phase in one agent run.
4. Run staged work **one stage at a time** (P0–P9 / D1–D14).
5. Open a **fresh chat** when moving to a substantially different stage.
6. Perform G9 / production-readiness board prep in a **fresh context** after freeze candidates exist.
7. **Measure before claiming** latency, cost, eval, clean-room, or production-ready targets.
8. Preferred agent build order: RTM(09) → domain(05–08) → contracts/resolver → failing prohibited tests → WF A→C→B → scripts/offline → app → eval stack → FinOps/gov → freeze → clean-room.
...
| A-002 | Decision | Jurisdiction/purpose/role/boundary/risk class/assumptions stated on **every** regulatory conclusion (not once-and-forget) | P4 | Every reg claim |
...
- **Must (never cut):** 3 offline workflows; prohibited-action fail-closed tests; 4 evidence files with §7.3 schemas; 4 runbooks (prereq/IO/failure/reset); 5 name-matched scripts **plus evidence-export capability**; `--final`; defence elements 3,4,5,7,13; agent freeze until G4; dual-track honesty; workflow-specific AI-disabled continuity; Cursor-oriented evidence **or recorded equivalent**; hard gates independent of score; deliberate-ambiguity preservation; facilitator independence.
- **Should:** full 12-suite depth (never drop suites 3,4,5,9,12); human rubric + regression history; ≥10 ADRs; subgroup breadth; Track B if production-ready claim needed; finops_delivery_log.csv daily.
- **Could:** PDF exports; UI polish; deep 19/20 where applicability argued; leadership PPTX polish.
- **Won’t:** real regulated use; editing challenge evidence; editing MANIFEST/FILE_HASHES to force green; claiming ISO/GxP certification; forcing KG/agent/LLM without justification; claiming production-ready without G9; importing AppFlowy Master Build Guide Gantt-planner product scope into AEGIS.
...
8. Ratify this **V2** plan (`AEGIS_PROJECT_PLAN_V2.md`).

---

## 18. Clean-room procedure (G7; repeat on RC for G9)

Per `runbooks/REPO_EXECUTION.md` + DoD evidence-export, outsider member:

1. Extract package to a **different directory**.  
2. Run preflight **twice** (detect generated-file drift).  
3. Run documented setup / run / test / evaluate / reset / **export** with `python -B`.  
...
7. Confirm no secrets / live PHI / undeclared network runtime dependency on the RC path.
...
- [ ] Every hard gate has evidenced “cannot occur” test (independent of 180-pt score)  
- [ ] 84 TC mirrored results ≠ `NOT_RUN` or declared gap  
- [ ] Eval human rubric + regression history present; LLM-judge controls if used  
- [ ] Evidence-export demonstrated in clean-room  
- [ ] Continuity: batch/supply 14d; PV 0h AI-outage window honored  
- [ ] 13 defence elements have presenters  
- [ ] Manifest complete including equivalent-evidence / cuts  
- [ ] Cursor-oriented evidence or recorded equivalent under `submission/evidence/`  
...
1. Fill seat names; ratify **V2** at P0.  
2. Archive Hour-0 verifier PASS.  
3. Create `team_charter.md` + assumptions/decision/RAID logs.  
4. Mirror `ProjectPlan/` SoT files into `submission/artefacts/ProjectPlan/`.  
5. `python -B tools/check_submission_structure.py --scaffold`.  
6. Start artefact 01 from BR-01 / kpi_conflicts / no_ai_baselines / INJ-001…006 — cite paths; no conflict cleaning.  
7. Book G1 (hour 7) review.  
8. Lock production NFRs table into decision log (even if Track B deferred).  
9. Seed `submission/evidence/finops_delivery_log.csv` with §11.9 header.
...
| Eval human rubric / judge / regression | §11.4 | Deliverables present | `submission/evaluation/` | Pending |
| Evidence-export + §7.3 schemas | §§7.2–7.3, 18 | Clean-room export + JSON fields | `submission/scripts/`, `submission/evidence/` | Pending |
| Continuity windows | Constraint 5 | Batch/supply 14d; PV 0h | runbooks + tests | Pending |
...
| AppFlowy AGENTS principles + Master Build Guide | Engineering principles missing; Gantt planner product out of scope; Cursor session method not written | Added `engineering-principles.mdc` + `APPFLOWY_PRINCIPLES_VALIDATION.md`; FINAL §15 cross-refs only | 2026-08-05 |
| Repo deep parse + AppFlowy re-validation | Path ghosts; PV continuity oversimplify; export/schema/eval rubric gaps; MoSCoW RUB-08 ambiguity | **V2.0** governing SoT; §24 G-24…G-38; sibling validation + daywise MD | 2026-08-05 |
| Daywise Excel vs Plan V2 (5 seats) | v1 tracker ratified FINAL; missing export/PV-0h/rubric/runbooks/Cursor method | **`AEGIS_FDE_DAYWISE_TASK_TRACKER_V2.xlsx`** (96 tasks; Gap_Log sheet) | 2026-08-05 |

---

## 24. V2 gap closures (repo SoT + AppFlowy)

| ID | Gap found | Correction in V2 |
|---|---|---|
| G-24 | FINAL referenced `MERGE_AUDIT.md`, `PROMPT_FINALIZE…`, daywise `.md`, PPTX under `submission/…` but files missing / paths wrong | Living SoT = `ProjectPlan/`; daywise MD created; validation sibling replaces missing MERGE_AUDIT; PPTX marked **owed** |
| G-25 | Blanket “14-day AI-disabled” ignored PV `max_ai_outage_hours=0` | Constraint §5 + stop table + Track A exit checklist honor `continuity_requirements.csv` |
| G-26 | Evidence-export treated as optional “(+ export)” vs DoD/REPO Must | Scripts row + MoSCoW + clean-room step 3 require export capability |
| G-27 | `test_results.json` / `evaluation_results.json` field schemas omitted | New §7.3 from `SUBMISSION_EVIDENCE_STANDARD.md` |
| G-28 | Eval human rubric / LLM-judge controls / regression history omitted | §11.4 Must deliverables |
| G-29 | Facilitator independence + deliberate ambiguity not non-negotiable | Constraints 10–11 |
| G-30 | Stack freedom (KG/agent/LLM not mandatory) weak | Constraint 12 + MoSCoW Won’t |
| G-31 | Regulatory framing weaker than package (“once per WF”) | Constraint 13; A-002 upgraded to every conclusion |
| G-32 | Hard gates vs 180-pt score not explicit | Constraint 14 + Track A checklist |
| G-33 | Cursor evidence MoSCoW “Could” vs ARTEFACT Must | §15 Must + MoSCoW Must |
| G-34 | “Cut depth on 08” ambiguous with RUB-08 | §12 clarification |
| G-35 | AppFlowy Cursor operating method only in sibling | Embedded §15.1; engineering principles constraint 15 |
| G-36 | Dual-track/G9 could be misread as package SoT | Document control + constraint 8: G9 = plan DECISION for prod-ready claim only |
| G-37 | Package `runbooks/` (PARTICIPANT/REPO) ≠ submission four runbooks | §7.2 clarifies challenge guides vs owed `submission/runbooks/` SETUP/OPERATIONS/INCIDENT/AI_DISABLED |
| G-38 | `MANIFEST.md` omits `engineering-principles.mdc` / `ProjectPlan/` | **Do not edit** immutable `MANIFEST.md`; document drift here. Post-work `verify_package.py` FAIL remains expected (§4.2). Team assets live under `ProjectPlan/` + `.cursor/rules/` |

**AppFlowy Master Build Guide product scope (WBS/Gantt planner, IndexedDB, 2000-task UI, Vite/Playwright mandates) remains explicitly out of AEGIS** — transferable operating/FinOps/readiness discipline only.

**Path inventory (living vs owed) — ProjectPlan/**

| Path | Status |
|---|---|
| `AEGIS_PROJECT_PLAN_V2.md` | **SoT (this file)** |
| `AEGIS_PROJECT_PLAN_FINAL.md` | Superseded audit copy |
| `AEGIS_PROJECT_TIMELINE_V2.png` | Present (SoT visual) |
| `AEGIS_PROJECT_TIMELINE_FINAL.png` | Legacy alias (same image) |
| `AEGIS_PROJECT_SWIMLANE_V2.md` | Present (seat × phase / day swimlanes) |
| `AEGIS_PROJECT_SWIMLANE_TRACKER_V2.xlsx` | Present (phase/day matrices + task trackers) |
| `_build_swimlane_tracker_v2.py` | Regenerates swimlane Excel V2 |
| `AEGIS_FDE_DAYWISE_TASK_DISTRIBUTION.md` | Present (V2) |
| `AEGIS_FDE_DAYWISE_TASK_TRACKER_V2.xlsx` | **Working tracker SoT** (96 tasks; 5 seats P1–P5) |
| `AEGIS_FDE_DAYWISE_TASK_TRACKER.xlsx` | Superseded v1 (audit) |
| `_build_daywise_tracker_v2.py` | Regenerates tracker V2 |
| `APPFLOWY_PRINCIPLES_VALIDATION.md` | Present (delta notes) |
| `REPO_APPFLOWY_VALIDATION_V2.md` | Present (this validation record) |
| Leadership / 5-slide PPTX | **Owed** before G8 |
